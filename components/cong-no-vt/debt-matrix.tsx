...
export function DebtMatrix({ rows, entities }: Props) {
  if (rows.length === 0) {
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-muted">
            <th className="border p-2 text-left min-w-[140px]" rowSpan={2}>NCC</th>
            {entities.map((e) => (
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.partyId} className="hover:bg-muted/30">
              <td className="border p-2 font-medium">{row.partyName || `NCC #${row.partyId}`}</td>
              {entities.map((e) => {
