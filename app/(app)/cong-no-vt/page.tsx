  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Công nợ Vật tư</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Tổng hợp công nợ TT / HĐ theo Chủ thể × Nhà cung cấp × Dự án
          </p>
        </div>
        <nav className="flex flex-wrap gap-1.5" aria-label="Điều hướng công nợ vật tư">
          {navLinks.map((l) => (
            <Link
              key={l.href}
              href={l.href}
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-muted/40">
              <tr>
                ...
                <th className="border-b px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">NCC</th>
              ...
              {summaryRows.length === 0 ? (
                ...
                summaryRows.map((r, idx) => (
                  <tr key={idx} className="even:bg-muted/20 hover:bg-muted/40 transition-colors">
                    ...
                    <td className="border-b px-3 py-2">{supplierMap[r.partyId] ?? `#${r.partyId}`}</td>
