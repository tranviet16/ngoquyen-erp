...
### Added
- Added product-wide three-state table sorting (`default → ascending → descending → default`) across DataTable, DataGrid, raw HTML, grouped/tree, matrix, and nested report families. Sorting follows displayed semantic values, keeps server pagination global, preserves group/subtotal/footer and matrix-axis structure, and uses accessible keyboard/touch headers where the renderer supports native table semantics.
- Dự Toán / DT Điều Chỉnh tabs now render as 2-level grouped tables (HM → VL/NC/Máy/CPC) with section/HM subtotals and % share (shared pure helper parses `HM<n>-<section>` category codes; non-conforming codes degrade to one level); Dự Toán keeps CRUD dialogs plus quick inline edit for SL/Đơn giá; Phát Sinh gained a per-category totals tree (incl. "Chưa gán hạng mục" bucket and approved-only totals) above its grid.
- Cân Đối VT reconciliation aids: expandable estimate rows listing member transactions, a "tên khác DT" badge (token-based name-mismatch detection via shared `normVtName`, now extracted to `lib/text/`), and a "Gán vào DT…" action that bulk-reassigns an "ngoài DT" transaction cluster to a chosen estimate row with a note trace.
- Substitute-material groups (`ProjectMaterialGroup` + `ProjectEstimate.materialGroupId`, per-project): multi-select grouping UI in Định Mức, group-level norm flags (member flags muted), quantity comparison only when member units match — otherwise money-only; group labels surface in Cân Đối VT's "Thi công vs DT" mode. Views `vw_project_norm`/`vw_project_estimate_adjusted` untouched — rollups are TS post-processing.
- Added the "Dự toán — Tổng hợp VT" import adapter (`du-toan-tong-hop-vt`): reads the official BOQ workbook's "Tổng hợp VT" sheet (Khối lượng × Giá T.B — switched from Giá gốc on 2026-08-20 per user decision), updates MNTC-GD1 estimates in place by normalized name+unit matching (invoice links and overrides preserved), inserts file-only items, keeps system-only rows with a KEEP report, and reconciles per-block totals against the sheet's own "TỔNG" rows (±2%, amount-only "%" rows counted toward the gate but not imported). Idempotent — re-running the same file is safe. Applied to MNTC-GD1: dự toán total 8.373 tỷ theo Giá T.B (khớp bảng cân đối gốc), 333+26 rows updated/inserted / 3 kept.
...
### Fixed
- Prevented the shared Glide DataGrid selection callback from causing a React maximum-update-depth loop when callers recreate rows or columns; sorting and filtering now clear index-based selection before changing the visible row order.
